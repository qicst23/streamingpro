...
import org.apache.spark.{OptimizeTaskContext, SparkPlanExecutor}
...
class RestPredictController extends ApplicationController {
  ...
  def compute = {
    ...
    val df1 = sparkSession.sql(param("sql"))
    val rs = SparkPlanExecutor.exec(df1.queryExecution.sparkPlan, sparkSession)
    val dateTypes = df1.schema.map(f => f.dataType)
    val list = rs.map(iRow => iRow.toSeq(dateTypes))
    println(list)
